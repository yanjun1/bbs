		...
		for ( var j = 0; j < relatedlink.length; j++) {
			if (relatedlink[j] && !relatedid[j]) {
				if (relatedlink[j]['surl'] != '') {
					var ra = '<a href="' + relatedlink[j]['surl']
							+ '" target="_blank" class="relatedlink">'
							+ relatedlink[j]['sname'] + '</a>';
					alink[alink_i] = ra;
					ra = '#alink ' + alink_i + '#';
					alink_i++;
				} else {
					var ra = '<strong><font color="#FF0000">'
							+ relatedlink[j]['sname'] + '</font></strong>';
				}
				var $rtmp = $3;
...
function _updatesecqaa(idhash) {
	if ($('secqaa_' + idhash)) {
		...
		$('checksecqaaverify_' + idhash).innerHTML = '<img src="' + IMGDIR
				+ '/none.gif" width="16" height="16" class="vm" />';
		ajaxget(
				'misc.php?mod=secqaa&action=update&idhash=' + idhash,
				'secqaa_' + idhash,
				null,
				'',
				'',
				function() {
					secST['qaa_' + idhash] = setTimeout(
							function() {
								$('secqaa_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updatesecqaa(\''
										+ idhash + '\')">刷新验证问答</span>';
							}, 180000);
				});
	}
...
function _updateseccode(idhash, play) {
	if (isUndefined(play)) {
		if ($('seccode_' + idhash)) {
			...
			$('checkseccodeverify_' + idhash).innerHTML = '<img src="' + IMGDIR
					+ '/none.gif" width="16" height="16" class="vm" />';
			ajaxget(
					'misc.php?mod=seccode&action=update&idhash=' + idhash,
					'seccode_' + idhash,
					null,
					'',
					'',
					function() {
						secST['code_' + idhash] = setTimeout(
								function() {
									$('seccode_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updateseccode(\''
											+ idhash + '\')">刷新验证码</span>';
								}, 180000);
					});
		}
	} else {
		eval('window.document.seccodeplayer_' + idhash
				+ '.SetVariable("isPlay", "1")');
	}
}
function _checksec(type, idhash, showmsg, recall) {
	...
	x.loading = '';
	$('checksec' + type + 'verify_' + idhash).innerHTML = '<img src="' + IMGDIR
			+ '/loading.gif" width="16" height="16" class="vm" />';
	x
			.get(
					'misc.php?mod=sec'
							+ type
							+ '&action=check&inajax=1&&idhash='
							+ idhash
							+ '&secverify='
							+ (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify)
									: secverify),
					function(s) {
						var obj = $('checksec' + type + 'verify_' + idhash);
						obj.style.display = '';
						if (s.substr(0, 7) == 'succeed') {
							obj.innerHTML = '<img src="'
									+ IMGDIR
									+ '/check_right.gif" width="16" height="16" class="vm" />';
							if (showmsg) {
								recall(1);
							}
						} else {
							obj.innerHTML = '<img src="'
									+ IMGDIR
									+ '/check_error.gif" width="16" height="16" class="vm" />';
							if (showmsg) {
								if (type == 'code') {
...
function _showdistrict(container, elems, totallevel, changelevel, containertype) {
	...
	var pid = changelevel >= 1 && elems[0] && $(elems[0]) ? getdid($(elems[0]))
			: 0;
	var cid = changelevel >= 2 && elems[1] && $(elems[1]) ? getdid($(elems[1]))
			: 0;
	var did = changelevel >= 3 && elems[2] && $(elems[2]) ? getdid($(elems[2]))
			: 0;
	var coid = changelevel >= 4 && elems[3] && $(elems[3]) ? getdid($(elems[3]))
			: 0;
	var url = "home.php?mod=misc&ac=ajax&op=district&container=" + container
			+ "&containertype=" + containertype + "&province=" + elems[0]
			+ "&city=" + elems[1] + "&district=" + elems[2] + "&community="
			+ elems[3] + "&pid=" + pid + "&cid=" + cid + "&did=" + did
			+ "&coid=" + coid + '&level=' + totallevel + '&handlekey='
			+ container + '&inajax=1' + (!changelevel ? '&showdefault=1' : '');
	...
	if (BROWSER.ie) {
		...
		var msg = '<div class="c"><div style="width: 200px; text-align: center; text-decoration:underline;">点此复制到剪贴板</div>'
				+ AC_FL_RunContent('id', 'clipboardswf', 'name',
						...
						'height', '40', 'src', STATICURL
								+ 'image/common/clipboard.swf', 'menu',
						'false', 'allowScriptAccess', 'sameDomain',
	...
	var showselect_row = function(inpid, s, v, notime, rettype) {
		if (v >= 0) {
			if (!rettype) {
				...
				return '<a href="javascript:;" onclick="$(\'' + inpid
		...
		if (!t) {
			...
			if ($(t)) {
				...
				for (i = 0; i < lis.length; i++) {
					s += '<a href="javascript:;" onclick="$(\'' + inpid
							+ '\').value = this.innerHTML;$(\'' + obj.id
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	var loadCheck = function(obj) {
		if (obj.complete) {
			...
			if (showexif && faid) {
				var x = new Ajax();
				x.get('forum.php?mod=ajax&action=exif&aid=' + faid
						+ '&inajax=1', function(s, x) {
					if (s) {
	...
	var showloading = function(zimg, pn) {
		...
		$(menuid + '_waiting').style.left = (document.body.clientWidth - 42)
				/ 2 + 'px';
		$(menuid + '_waiting').style.top = ((document.documentElement.clientHeight - 42) / 2 + Math
	...
	var showimage = function(zimg, w, h, imgw, imgh) {
		...
		$(menuid + '_zoomlayer').style.height = (mheight < 120 ? 120 : mheight)
				+ 'px';
		$(menuid + '_img').innerHTML = '<img id="' + zoomid + '" src="' + zimg
				+ '" width="' + w + '" height="' + h + '" w="' + imgw + '" h="'
				+ imgh + '" />' + imgtitle;
		if ($(menuid + '_imglink')) {
	...
	var adjust = function(e, a) {
		...
		menu.style.width = (parseInt(ele.width < 300 ? 300
				: parseInt(ele.width)) + 20)
				+ 'px';
...
function _zoom_page(paid, showexif) {
	var imagesrc = $('aimg_' + paid).getAttribute('zoomfile') ? $(
			'aimg_' + paid).getAttribute('zoomfile') : $('aimg_' + paid)
			.getAttribute('file');
...
function _switchTab(prefix, current, total, activeclass) {
	...
	for ( var i = 1; i <= total; i++) {
		...
		$(prefix + '_' + i).className = classname.replace(
				' ' + activeclass + ' ', '').substr(1);
		$(prefix + '_c_' + i).style.display = 'none';
	}
	$(prefix + '_' + current).className = $(prefix + '_' + current).className
			+ ' ' + activeclass;
	$(prefix + '_c_' + current).style.display = '';
}
function _initTab(frameId, type) {
	if (typeof document['diyform'] == 'object'
			|| $(frameId).className.indexOf('tab') < 0)
		return false;
	type = type || 'click';
	...
	for ( var i in tabs) {
		if (tabs[i]['nodeType'] == 1
				&& tabs[i]['className'].indexOf('move-span') > -1) {
			arrTab.push(tabs[i]);
		}
	}
	var counter = 0;
	...
	for ( var i = 0; i < len; i++) {
		...
		if (hasClass(arrTab[i], 'frame') || hasClass(arrTab[i], 'tab')) {
			...
			for ( var j in arrTab[i].childNodes) {
				if (typeof arrTab[i].childNodes[j] == 'object'
						&& !hasClass(arrTab[i].childNodes[j], 'title'))
					arrColumn.push(arrTab[i].childNodes[j]);
			}
			var frameContent = document.createElement('div');
			frameContent.id = tabId + '_content';
			frameContent.className = hasClass(arrTab[i], 'frame') ? 'content cl '
					+ arrTab[i].className.substr(arrTab[i].className
							.lastIndexOf(' ') + 1)
					: 'content cl';
		...
		li.innerHTML = arrTab[i]['innerText'] ? arrTab[i]['innerText']
				: arrTab[i]['textContent'];
		var a = arrTab[i].getElementsByTagName('a');
		var href = a && a[0] ? a[0].href : 'javascript:;';
		var onclick = type == 'click' ? ' onclick="return false;"' : '';
		li.innerHTML = '<a href="' + href + '"' + onclick
				+ ' onfocus="this.blur();" ' + style + '>' + imgs
				+ li.innerHTML + '</a>';
...
function switchTabUl(e) {
	...
	for ( var j = 0; j < len2; j++) {
		...
		if (content)
			content.style.display = tabs[j].id == tabId ? '' : 'none';
	}
}
function slideshow(el) {
	...
	this.slideshows = !this.slideshows ? filterTextNode(el.childNodes)
			: filterTextNode(this.slideshows);
	this.length = this.slideshows.length;
	for (i = 0; i < this.length; i++) {
		this.slideshows[i].style.display = "none";
		_attachEvent(this.slideshows[i], 'mouseover', function() {
			obj.stop();
		});
		_attachEvent(this.slideshows[i], 'mouseout', function() {
			obj.goon();
		});
	}
	...
	if (!this.slidebar) {
		...
		for (i = 0; i < this.controls.length; i++) {
			...
			_attachEvent(this.controls[i], 'mouseout', function() {
				obj.goon();
			});
		}
	}
...
function slidexactive(step) {
	...
	currentslideele = hasClass(aim, 'slidebarup')
			|| hasClass(aim, 'slidebardown') || hasClass(parent, 'slidebar') ? aim
			: null;
	while (parent && parent != document.body) {
		...
		if (!currentslideele
				&& (hasClass(parent, 'slidebarup') || hasClass(parent,
						'slidebardown'))) {
			currentslideele = parent;
		}
	...
	var children = slidebar.length == 0 ? []
			: filterTextNode(slidebar[0].childNodes);
	if (currentslideele
			&& (hasClass(currentslideele, 'slidebarup') || hasClass(
					currentslideele, 'slidebardown'))) {
		xactivei = step;
	} else {
		for ( var j = 0, i = 0, L = children.length; i < L; i++) {
			...
			if (!hasClass(children[i], 'slidebarup')
					&& !hasClass(children[i], 'slidebardown'))
				j++;
		}
	}
	if (slideboxid != null && xactivei != null)
		slideshow.entities[slideboxid].xactive(xactivei);
}
...
function _showTip(ctrlobj) {
	...
	if (!$(menuid)) {
		...
		div.style.display = 'none';
		div.innerHTML = '<div class="tip_horn"></div><div class="tip_c">'
				+ ctrlobj.getAttribute('tip') + '</div>';
		$('append_parent').appendChild(div);
	}
...
function _showPrompt(ctrlid, evt, msg, timeout, classname) {
	...
	if (ctrlid) {
		msg = '<div id="' + ctrlid
				+ '_prompt"><div class="tip_horn"></div><div class="tip_c">'
				+ msg + '</div>';
	} else {
		msg = '<table cellspacing="0" cellpadding="0" class="popupcredit"><tr><td class="pc_l">&nbsp;</td><td class="pc_c"><div class="pc_inner">'
				+ msg + '</td><td class="pc_r">&nbsp;</td></tr></table>';
	}
	div.innerHTML = msg;
	if (ctrlid) {
		if (!timeout) {
...
function _showCreditPrompt() {
	...
	var creditrule = decodeURI(getcookie('creditrule', 1)).replace(
...
function creditShow(creditinfo, notice, basev, bk, first, creditrule) {
	...
	for (i = 1; i <= 8; i++) {
		...
		if (notice[i] !== '0' && creditinfo[i]) {
			s += '<span>'
					+ creditinfo[i][0]
					+ (notice[i] != 0 ? (notice[i] > 0 ? '<em>+'
							: '<em class="desc">')
							+ notice[i] + '</em>' : '') + creditinfo[i][1]
					+ '</span>';
		}
		if (notice[i] > 0) {
	...
	if (!$('creditpromptdiv')) {
		showPrompt(null, null, '<div id="creditpromptdiv">'
				+ (creditrule ? '<i>' + creditrule + '</i> ' : '') + s
				+ '</div>', 0);
	} else {
...
function _showColorBox(ctrlid, layer, k, bgcolor) {
	var tag1 = !bgcolor ? 'color' : 'backcolor', tag2 = !bgcolor ? 'forecolor'
			: 'backcolor';
	if (!$(ctrlid + '_menu')) {
		...
		var coloroptions = [ 'Black', 'Sienna', 'DarkOliveGreen', 'DarkGreen',
		...
		var colortexts = [ '黑色', '赭色', '暗橄榄绿色', '暗绿色', '暗灰蓝色', '海军色', '靛青色',
				'墨绿色', '暗红色', '暗桔黄色', '橄榄色', '绿色', '水鸭色', '蓝色', '灰石色', '暗灰色',
				'红色', '沙褐色', '黄绿色', '海绿色', '间绿宝石', '皇家蓝', '紫色', '灰色', '红紫色',
				'橙色', '黄色', '酸橙色', '青色', '深天蓝色', '暗紫色', '银色', '粉色', '浅黄色',
				'柠檬绸色', '苍绿色', '苍宝石绿', '亮蓝色', '洋李色', '白色' ];
		var str = '';
		for ( var i = 0; i < 40; i++) {
			str += '<input type="button" style="background-color: '
					...
					+ (typeof wysiwyg == 'undefined' ? 'seditor_insertunit(\''
							+ k + '\', \'[' + tag1 + '=' + coloroptions[i]
							+ ']\', \'[/' + tag1 + ']\')' : (ctrlid == editorid
							+ '_tbl_param_4' ? '$(\'' + ctrlid + '\').value=\''
							+ coloroptions[i] + '\';hideMenu(2)'
							: 'discuzcode(\'' + tag2 + '\', \''
...
function _toggle_collapse(objname, noimg, complex, lang) {
	...
	if (!noimg) {
		...
		if (img.tagName != 'IMG') {
			...
			img.src = img.src.indexOf('_yes.gif') == -1 ? img.src.replace(
					/_no\.gif/, '_yes\.gif') : img.src.replace(/_yes\.gif/,
					'_no\.gif');
		}
		img.blur();
	}
...
function _extstyle(css) {
	...
	$('css_extstyle').href = css ? css + '/style.css' : STATICURL
			+ 'image/common/extstyle_none.css';
	currentextstyle = css;
...
function _showCreditmenu() {
	if (!$('extcreditmenu_menu')) {
		...
		menu.innerHTML = '<div class="p_opt"><img src="'
				+ IMGDIR
				+ '/loading.gif" width="16" height="16" class="vm" /> 请稍候...</div>';
		$('append_parent').appendChild(menu);
...
function _showUpgradeinfo() {
	if (!$('g_upmine_menu')) {
		...
		menu.innerHTML = '<div class="p_opt"><img src="'
				...
				+ '/loading.gif" width="16" height="16" class="vm" /> 请稍候...</div>';
		$('append_parent').appendChild(menu);
		ajaxget('home.php?mod=spacecp&ac=usergroup&showextgroups=1',
				'g_upmine_menu', 'ajaxwaitid');
	}
	showMenu({
		'ctrlid' : 'g_upmine',
		'ctrlclass' : 'a',
...
function _showForummenu(fid) {
	if ($('fjump_menu') && !$('fjump_menu').innerHTML) {
		ajaxget('forum.php?mod=ajax&action=forumjump&jfid=' + fid,
				'fjump_menu', 'ajaxwaitid');
	}
}
function _showUserApp(fid) {
	var menu = $('mn_userapp_menu');
	if (menu && !menu.innerHTML) {
		ajaxget('misc.php?mod=manyou&action=menu', 'mn_userapp_menu',
				'ajaxwaitid');
	}
}
function _imageRotate(imgid, direct) {
	...
	if (BROWSER.ie) {
		if (!isNaN(image.getAttribute('om'))) {
			image.style.marginBottom = (image.getAttribute('om') + (BROWSER.ie < 8 ? 0
					: (deg == 90 || deg == 270 ? Math.abs(ow - oh) : 0)))
					+ 'px';
		}
		image.style.filter = 'progid:DXImageTransform.Microsoft.BasicImage(rotation='
				+ (deg / 90) + ')';
	} else {
		switch (deg) {
		case 90:
			var cow = oh, coh = ow, cx = 0, cy = -oh;
			break;
		case 180:
			var cow = ow, coh = oh, cx = -ow, cy = -oh;
			break;
		case 270:
			var cow = oh, coh = ow, cx = -ow, cy = 0;
			break;
		}
...
function _createPalette(colorid, id, func) {
	var iframe = "<iframe name=\"c"
			+ colorid
			+ "_frame\" src=\"\" frameborder=\"0\" width=\"210\" height=\"148\" scrolling=\"no\"></iframe>";
	if (!$("c" + colorid + "_menu")) {
	...
	func = !func ? '' : '|' + func;
	window.frames["c" + colorid + "_frame"].location.href = SITEURL + STATICURL
			+ "image/admincp/getcolor.htm?c" + colorid + "|" + id + func;
	showMenu({
		'ctrlid' : 'c' + colorid
	});
...
function _setShortcut() {
	$('shortcuttip').onclick = function() {
		...
		msg += '2、点击"'
				+ '<a href="forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
